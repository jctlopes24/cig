...
function verificarAutenticacao() {
    ...
    if (!currentUserStr) {
        ...
        alert('Precisa de iniciar sessão para fazer uma reserva.');
...
function carregarHotel() {
    ...
    if (reservaDataStr) {
        ...
        if (reservaData.checkin) document.getElementById('checkin').value = reservaData.checkin;
        if (reservaData.checkout) document.getElementById('checkout').value = reservaData.checkout;
        if (reservaData.hospedes) document.getElementById('hospedes').value = reservaData.hospedes;
        if (reservaData.tipoQuarto) document.getElementById('tipoQuarto').value = reservaData.tipoQuarto;
...
function confirmarReserva() {
    // Verificar autenticação antes de confirmar
    ...
    // Atualizar histórico do utilizador
    const currentUserStr = localStorage.getItem('currentUser');
    if (currentUserStr) {
        const currentUser = JSON.parse(currentUserStr);
        const users = JSON.parse(localStorage.getItem('users') || '[]');
        ...
        for (let i = 0; i < users.length; i++) {
            if (users[i].email === currentUser.email) {
                if (!users[i].perfil) users[i].perfil = { historicoReservas: [] };
                if (!users[i].perfil.historicoReservas) users[i].perfil.historicoReservas = [];
